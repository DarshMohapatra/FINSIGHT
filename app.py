...
.stTextInput>div>div>input{background:rgba(255,255,255,0.04)!important;color:white!important;border:1px solid rgba(255,255,255,0.1)!important;border-radius:10px!important;padding:12px 16px!important;}
...
def cfmt(x, pos):
    raw = abs(x)
    if raw >= 1e7:   return f"Rs.{x/1e7:.1f}Cr"
    elif raw >= 1e5: return f"Rs.{x/1e5:.1f}L"
    elif raw >= 1e3: return f"Rs.{x/1e3:.0f}K"
    else:            return f"Rs.{x:.0f}"

def build_top5_text(df):
    top5 = df.nlargest(5, "WITHDRAWAL AMT")
    lines = []
    for _, row in top5.iterrows():
        date_str   = row["DATE"].strftime("%d %b %Y")
        desc_str   = str(row["TRANSACTION DETAILS"])[:40]
        amt_str    = cfmt(row["WITHDRAWAL AMT"], None)
        lines.append("  - " + date_str + " | " + desc_str + " | " + amt_str)
    return "
".join(lines)
...
def build_context(df):
    ...
    top5_text   = build_top5_text(df)
    trend_text  = " | ".join([str(m) + ": " + str(round(v/scale, 2)) for m, v in monthly_t.items()])
    cat_text    = "
".join([
        "  - " + c + ": " + str(round(a/scale, 2)) + " " + slbl
    ...
    flow_label = "SURPLUS" if total_dep > total_wd else "DEFICIT"

    ctx  = "You are FinSight, a smart personal AI financial advisor.
"
    ctx += "REAL USER BANK DATA (all amounts in " + slbl + "):
"
    ctx += "Date Range: " + date_min + " to " + date_max + "
"
    ctx += "Total Transactions: " + str(len(df)) + "
"
    ctx += "Total Withdrawals: " + str(round(total_wd,2)) + " " + slbl + "
"
    ctx += "Total Deposits: "    + str(round(total_dep,2)) + " " + slbl + "
"
    ctx += "Net Flow: " + str(round(total_dep-total_wd,2)) + " " + slbl + " (" + flow_label + ")
"
    ctx += "Avg Monthly Spending: " + str(round(avg_monthly,2)) + " " + slbl + "
"
...
.btn{display:inline-flex;align-items:center;gap:10px;padding:16px 32px;border-radius:12px;font-weight:700;font-size:15px;text-decoration:none;transition:all 0.2s;cursor:pointer;font-family:'Syne',sans-serif;}
...
<section class="section" id="features"><div class="wrap">
    ...
    <div class="grid4 reveal">
        <div class="feat"><div class="ficon ig">🧠</div><div class="fn">01 / 04</div><div class="ft">NLP Transaction Categorizer</div><div class="fd">Every transaction auto-labeled — transfers, payments, salary, EMIs — using keyword-aware NLP. 78.4% coverage out of the box.</div><div class="tags"><span class="tag">NLP</span><span class="tag">KEYWORD MATCHING</span><span class="tag">AUTO-LABEL</span></div></div>
        <div class="feat"><div class="ficon ir">🚨</div><div class="fn">02 / 04</div><div class="ft">Anomaly Detection Engine</div><div class="fd">Isolation Forest ML model scans 100K+ transactions and flags the suspicious 2% — unusual amounts, timing spikes, pattern breaks.</div><div class="tags"><span class="tag">ISOLATION FOREST</span><span class="tag">SKLEARN</span><span class="tag">REAL-TIME</span></div></div>
        <div class="feat"><div class="ficon ib">📈</div><div class="fn">03 / 04</div><div class="ft">Expense Forecasting</div><div class="fd">Facebook Prophet predicts your next 6 months of spending with confidence intervals — displayed clearly, not in scientific notation.</div><div class="tags"><span class="tag">PROPHET</span><span class="tag">TIME SERIES</span><span class="tag">6-MONTH</span></div></div>
        <div class="feat"><div class="ficon ip">🤖</div><div class="fn">04 / 04</div><div class="ft">AI Financial Advisor</div><div class="fd">Powered by LLaMA 3.3 70B via Groq. Knows your actual spending data. Gives personalized, data-backed advice — not generic tips.</div><div class="tags"><span class="tag">LLAMA 3.3</span><span class="tag">GROQ API</span><span class="tag">RAG</span></div></div>
    </div>
</div></section>
<section class="section" id="how"><div class="wrap">
    ...
    <div class="steps-wrap">
        <div class="step reveal"><div class="snum">01</div><div><div class="stag">UPLOAD</div><h3>Drop your bank statement</h3><p>Upload a CSV or Excel bank statement. Smart parser handles any format — HDFC, SBI, ICICI, Axis and more.</p></div></div>
        <div class="step reveal"><div class="snum">02</div><div><div class="stag">ANALYZE</div><h3>AI reads every transaction</h3><p>Three ML models run in parallel — NLP categorizer labels transactions, Isolation Forest hunts anomalies, Prophet forecasts spending.</p></div></div>
        <div class="step reveal"><div class="snum">03</div><div><div class="stag">VISUALIZE</div><h3>Your financial story, visualized</h3><p>Dark-themed dashboard with spending trends, category breakdown, anomaly timeline, and 6-month forecast — all in one view.</p></div></div>
        <div class="step reveal"><div class="snum">04</div><div><div class="stag">CHAT</div><h3>Ask anything about your money</h3><p>FinSight knows your real data and gives honest, personalized answers — powered by LLaMA 3.3 70B.</p></div></div>
    </div>
</div></section>
<section class="section" id="chat"><div class="wrap">
    ...
    <div class="chat-box reveal">
        ...
        <div class="messages">
            ...
            <div class="msg ai"><div class="bubble">Your biggest category is <strong>Online Payment</strong> at Rs.11,025Cr — 46% of total spending. Review your recurring NEFT transfers to find savings. 💡</div><div class="mt">FinSight AI</div></div>
            <div class="msg u"><div class="bubble">Give me 3 tips to reduce my spending!</div><div class="mt">Just now</div></div>
            <div class="msg ai"><div class="bubble">Based on your real data:<br><br>1. 🔍 <strong>Monitor Online Payments</strong> — Rs.11,025Cr. Review recurring transfers.<br>2. 💸 <strong>Consolidate Accounts</strong> — Rs.4,248Cr in transfers. Reduce account juggling.<br>3. 📊 <strong>Break down Other</strong> — Rs.1,504Cr untracked. Find hidden leaks.</div><div class="mt">FinSight AI</div></div>
        </div>
    </div>
</div></section>
<section class="section" id="cta"><div class="wrap">
    <div class="cta-box reveal">
        ...
        <p style="font-size:17px;color:rgba(232,234,240,0.45);margin-bottom:40px;line-height:1.7;">Upload your bank statement and get your complete AI-powered financial analysis in under 60 seconds.</p>
    </div>
</div></section>
...
if st.session_state.page == "landing":
    ...
    with c1:
        st.markdown("""<div style="padding:20px 0 12px;">
            <span style="font-size:28px;font-weight:800;letter-spacing:-1px;">⚡<span style="background:linear-gradient(135deg,#00f5a0,#00d4ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">FinSight</span></span>
                ...
                if st.button("⚡ Run Analysis", use_container_width=True):
                    with st.spinner("Running ML pipeline..."):
                        try:
                            ...
                            st.error("❌ " + str(e))
        with c2:
            st.markdown("""<div class="glass" style="margin-top:48px;">
                <div style="font-family:DM Mono,monospace;font-size:10px;color:#00f5a0;letter-spacing:2px;margin-bottom:16px;">AUTO-DETECTED COLUMNS</div>
                <table style="width:100%;border-collapse:collapse;font-size:12px;">
                <tr style="color:rgba(255,255,255,0.3);"><td style="padding:6px 0;font-family:DM Mono,monospace;">Required As</td><td style="padding:6px 0;font-family:DM Mono,monospace;">Also Accepted</td></tr>
                <tr style="border-top:1px solid rgba(255,255,255,0.06);"><td style="padding:8px 0;color:#00f5a0;font-family:DM Mono,monospace;font-size:11px;">DATE</td><td style="padding:8px 0;color:rgba(255,255,255,0.4);font-size:11px;">Transaction Date, Timestamp</td></tr>
                <tr style="border-top:1px solid rgba(255,255,255,0.06);"><td style="padding:8px 0;color:#00f5a0;font-family:DM Mono,monospace;font-size:11px;">TRANSACTION DETAILS</td><td style="padding:8px 0;color:rgba(255,255,255,0.4);font-size:11px;">Narration, Type, Category</td></tr>
                <tr style="border-top:1px solid rgba(255,255,255,0.06);"><td style="padding:8px 0;color:#00f5a0;font-family:DM Mono,monospace;font-size:11px;">WITHDRAWAL AMT</td><td style="padding:8px 0;color:rgba(255,255,255,0.4);font-size:11px;">Debit, Debit Amt, DR</td></tr>
                <tr style="border-top:1px solid rgba(255,255,255,0.06);"><td style="padding:8px 0;color:rgba(255,255,255,0.5);font-family:DM Mono,monospace;font-size:11px;">DEPOSIT AMT</td><td style="padding:8px 0;color:rgba(255,255,255,0.4);font-size:11px;">Credit, Credit Amt, CR</td></tr>
                <tr style="border-top:1px solid rgba(255,255,255,0.06);"><td style="padding:8px 0;color:rgba(255,255,255,0.5);font-family:DM Mono,monospace;font-size:11px;">BALANCE AMT</td><td style="padding:8px 0;color:rgba(255,255,255,0.4);font-size:11px;">Balance, Closing Balance</td></tr>
